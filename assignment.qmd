...
impeach %>%
  filter(party == "D") %>% 
  mutate(
    college_graduates = ifelse(pct_bachelors_compared_to_national > 1, "Above", "Below"),
    median_age = ifelse(median_age_compared_to_national > 1, "Above", "Below")
  ) %>% 
  group_by(for_impeachment, college_graduates, median_age) %>%
  summarise(count = n())
...
impeach %>%
  group_by(for_impeachment) %>%
  summarise(mean_pct_bachelors = mean(pct_bachelors))
...
impeach %>%
  ...
  group_by(margin_flag_2018 == "5_points_or_less") %>%
  summarise(count = n())
...
impeach <-
  impeach %>%
  mutate(year=substr(date_announced, 0,4), month = substr(date_announced, 6, 7))
